function [CE, gradients] = compute_CE_and_gradients_parfor(density_design,gc_region_index,chi,chi_max,flux_Pts,Ezinc_monitor,Hy_t,Pt,Einc, P_inc,k,ax,ay,nx,ny,xySim,h,Npv,Nph,NODES,param,quad_order,tol,maxit)
% This function calculates coupling efficiency (overlap integral) and the gradients
% for all wavelengths and angles
% Use parfor to speed up when Nk is large
chi(gc_region_index) = chi_max*density_design; % update the design region
Nk = size(P_inc,1);
Nth = size(P_inc,2);
CE = zeros(Nk,Nth);
gradients = zeros(length(density_design),Nk,Nth);
parfor i=1:Nk % use parfor to speed up
